...
def verify_internal_secret(req):
    ...
    if not INTERNAL_SECRET:
        raise RuntimeError("INTERNAL_SECRET is required")
...
def render_ai_video():
    ...
    try:
        ...
        if len(clips) > 20:
            return jsonify({'error': 'Max 20 clips allowed'}), 400
        total_dur = sum(seg.get('duration', 5) for seg in segments)
        if total_dur > 300:
            return jsonify({'error': 'Max total duration 300 seconds'}), 400

        # 1. Download all clips
        clip_paths = []
        max_clip_size = 50 * 1024 * 1024  # 50 MB per clip
        ...
        for i in range(len(clip_paths)):
            ...
            try:
                ...
                return jsonify({'error': f'Invalid segment duration for segment {i}'}), 400
            filter_parts.append(
